...
# densities should expect as argument theta
# proposals should take "cur" (current) and "prop" (proposed)
# RNG should expect requested number of samples, with default 1 sample
###############################################################################
...
MHSampler = R6::R6Class(
  ...
  public = list(
    ...
    run_chain = function(S=1000L,print_every=100L){
      ...
      if(is.infinite(cur_ld)) stop("log-target(theta_0)=-Inf, cannot proceed.")
      ...
      for(s in seq.int(2L,S)){
        ...
        res = self$ldtarget(ptheta) # compute prior and loglik at proposal
    ...
    # set theta_0 using output from run_chains_parallel
